...
# 7b. Determine which model won for the write-up
if advanced_mae < baseline_mae:
    performance_context = "While the Advanced model reduces error compared to a Naive baseline, absolute predictive certainty is impossible"
else:
    performance_context = "Interestingly, the Advanced trend model actually produced a higher forecast error than the simple Naive baseline. This proves that linear forecasting breaks down in highly volatile environments, which is entirely expected"

# 8. Construct Markdown Output
step_4_block = f"""
